...
cancer_patient as (
    select distinct ims_patient_id,diagnose_dt
    from patient_records
    where icd_type='2' and 4digit_icd='pancreatic_cancer'
)
 
select stage, count(distinct ims_patient_id)
from (
select * from (
select A.ims_patient_id, diagnose_dt, stage_id, stage, 
        CONCAT(SUBSTRING(stage_date, 8, 4), '-', 
                  CASE 
                    WHEN SUBSTRING(stage_date, 4, 3) = 'JAN' THEN '01'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'FEB' THEN '02'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'MAR' THEN '03'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'APR' THEN '04'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'MAY' THEN '05'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'JUN' THEN '06'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'JUL' THEN '07'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'AUG' THEN '08'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'SEP' THEN '09'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'OCT' THEN '10'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'NOV' THEN '11'
                    WHEN SUBSTRING(stage_date, 4, 3) = 'DEC' THEN '12'
                  END, '-', 
                  SUBSTRING(stage_date, 1, 2)) AS stage_dt
from cancer_patient as A
inner join IQVIA.IQVIA_ONCEMR_EMR_STAGE_HISTORY as B
on A.ims_patient_id=B.ims_patient_id
) as tmp
where datediff(stage_dt, diagnose_dt)<=30 and datediff(stage_dt, diagnose_dt)>=-30
) as Z
group by stage
