...
cancer_patient_records as (
    select distinct ims_patient_id, stage_id
    from patient_records
    where icd_type='2' and tumor_sub_type like '%SARCOMA' and 4digit_icd='sarcoma-trunk_extremities'
)
select
    stage, count(distinct ims_patient_id)
from (
    select *
    from (
        select *, ROW_NUMBER() OVER (PARTITION BY ims_patient_id ORDER BY stage DESC) AS rn
        from (
            select
                A.ims_patient_id, A.stage_id, B.stage
            from cancer_patient_records as A
            inner join IQVIA.IQVIA_ONCEMR_EMR_STAGE_HISTORY as B
            on A.ims_patient_id=B.ims_patient_id and A.stage_id=B.stage_id
        ) as C
    ) as D
    where rn=1
) as tmp
